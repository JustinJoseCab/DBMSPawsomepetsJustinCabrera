...
createClinicQuery = """
      ...
      staffNo TEXT CHECK(LENGTH(staffNo) = 5) UNIQUE, -- Ensuring one staff manages only one clinic
...
createPetQuery =  """
     ...
     pColor TEXT,
     clinicNo INT CHECK(clinicNo BETWEEN 0 AND 99),
     ownerNo INT CHECK(LENGTH(ownerNo) = 5) UNIQUE, -- Ensuring one pet has one owner
     PRIMARY KEY(petNo),
     FOREIGN KEY(clinicNo) REFERENCES Clinic(clinicNo),
...
#List all members of staff that work at a specified clinic address:
print("Query #1")
queryNumber1 = """
    SELECT s.*,c.cAddress
    FROM Staff s
...
queryNumber2 = """
    ...
    BestClinic AS (
        SELECT p.clinicNo, COUNT(ex.examNo) AS clinicVisits, strftime('%m', ex.dateSeen) AS month
        FROM Examination ex
        INNER JOIN Pet p ON ex.petNo = p.petNo
        GROUP BY p.clinicNo, month
    )
    SELECT bc.clinicNo, bc.clinicVisits, mf.month
    FROM BestClinic bc
    INNER JOIN MostFrequentMonth mf ON bc.month = mf.month
    ORDER BY bc.clinicVisits DESC
    LIMIT 1
"""
...
#3.	Which clinic has the most visits? 
print("Query #3")
queryNumber3 = """
...
#4. What staff have seen pets of a certain breed
print("Query #4")
queryNumber4 = """
